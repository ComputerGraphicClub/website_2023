...
for(let i = 0; i < archiveButton.length; i++){
    ...
    function OpenCloseDescription(a) {
        if (archiveButtonState === 0) {
            ...
            if (window.matchMedia("(min-width: 600px)").matches) sectionTitle[a].style.opacity = "0";
            else sectionTitle[a].style.opacity = "1";
        } else {
            ...
            if (window.matchMedia("(min-width: 600px)").matches) sectionTitle[a].style.opacity = "1";
            else sectionTitle[a].style.opacity = "1";
        }
    }
}
...
const infoArray = [
    "Edition - 24 \xd7 16.5 cm<br>104 pages <span>Download</span>",
    "Edition - 15.5 \xd7 11 cm<br>144 pages <span>Download</span>",
    "Term paper - 21 \xd7 14.5 cm<br>58 pages <span>Download</span>",
    "Term paper - 21 \xd7 14 cm<br>86 pages <span>Download</span>",
    "Research Paper - A4<br>16 pages <span>Download</span>",
    "Research Paper - A4<br>6 pages <span>Download</span>"
];
for(let i = 0; i < issues.length; i++)issues[i].onclick = ()=>{
    issues.forEach((el)=>{
...
function isInViewportX(el) {
    ...
    return rect.left >= (-window.innerWidth / 2 || -document.documentElement.clientWidth / 2) && rect.left <= (window.innerWidth / 2 || document.documentElement.clientWidth / 2);
}
const slide = document.querySelectorAll(".new-slides div");
const dot = document.querySelectorAll(".navigation-slides a");
console.log(slide.length);
document.addEventListener("scroll", function(ev) {
    for(let i = 0; i < slide.length; i++)if (isInViewportX(slide[i])) dot[i].classList.add("active");
    else dot[i].classList.remove("active");
}, true);
...
if (window.matchMedia("(min-width: 600px)").matches) {
    function isInViewportXissues(el) {
        const rect = el.getBoundingClientRect();
        return rect.left >= (window.innerWidth / 4 || document.documentElement.clientWidth / 4) && rect.left <= (window.innerWidth * (3 / 4) || document.documentElement.clientWidth * (3 / 4));
    }
    document.addEventListener("scroll", function(ev) {
        for(let i = 0; i < slideIssue.length; i++)if (isInViewportXissues(slideIssue[i])) dotIssue[i].classList.add("active");
        else dotIssue[i].classList.remove("active");
    }, true);
} else {
    function isInViewportXissues(el) {
        const rect = el.getBoundingClientRect();
...
function isInViewportY(el) {
    const rect = el.getBoundingClientRect();
    return rect.top >= (-window.innerHeight / 2 || -document.documentElement.clientHeight / 2) && rect.top <= (window.innerHeight / 2 || document.documentElement.clientHeight / 2);
}
...
document.addEventListener("scroll", function(ev) {
    for(let i = 0; i < section.length; i++)if (isInViewportY(section[i])) {
...
for (const htmlElement of allImageWithLazyLoading)if (htmlElement instanceof HTMLImageElement) {
    const imageWithLazyLoad = new ImageLazyLoad({
        htmlImageElement: htmlElement
    });
    document.addEventListener("scroll", ()=>{
        if (htmlElement.getBoundingClientRect().top > -100 && htmlElement.getBoundingClientRect().top < window.innerHeight) imageWithLazyLoad.loadImage({
            sizeOfImageToLoad: "reg"
        });
    }, true);
} else console.error(htmlElement, " need to be HTMLImageElement");
new CanvasGradation();
